...
function combineCards(
  ...
  combine(ocurrences, sizeCombination, 0, combinationCards);
}
...
function Game(rotationCard: string, pileParam: string[] = []) {
  ...
  function mapCardValueOcurrences() {
    const ocurrences = pile.reduce((acc: any, curr: string) => {
  ...
  const cardValueOcurrences = mapCardValueOcurrences();

  function groupByCombination(cardOcurrences: string[][]) {
    return cardOcurrences.reduce((acc: {3: string[][]; 2: string[][]}, curr) => {
      if (curr.length >= 3) {
  ...
  const groupCombinationCards = groupByCombination(Object.values(cardValueOcurrences));
